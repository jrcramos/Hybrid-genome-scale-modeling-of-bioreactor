...
for ib=1:data.nbatch
    ...
    control_nodes=zeros(1,numel(hnet.layers{hnet.nl}.x));
    for t=2:size(ybatch,1)
       ...
       if strcmp(hnet.layers{hnet.nl}.type,'Metabolic')
           ...
           control_nodes(t,:)=hnet.layers{hnet.nl-1}.y;
       end
%        v=data.batch(ib).rates_full(:,hnet.layers{hnet.nl}.Ir);
    ...
    if strcmp(hnet.layers{hnet.nl}.type,'Metabolic')
        ...
        control_nodes(1,:)=control_nodes(2,:);
        data.batch(ib).control_nodes=control_nodes;
    end
