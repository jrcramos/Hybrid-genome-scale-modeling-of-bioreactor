...
% material of this paper and the code to go from data.xlsx to data.mat is
% provided in '~/data/main_data_processing.m'
%---------------------------------------------------------------------------
...
%---------------------------------------------------------------------------
structures={};
niter=150000; % fixed for comparability
...
npcs= 4; % number of principal compenent for reaction correlation matrix, 
         % will not be used when 'dopca=2', i.e. when the genome scale is
         % known and is used as the last layer in the neural network structure
Indtr=repmat([1 2 3 4 9],nruns,1); % use ccdesign DoE square for hybnet training
Indcr=repmat([7],nruns,1); % use of 1 diamond edge of ccdesign DoE if cross-validation is performed
Indtt=repmat([5 6 8],nruns,1); % use the remaining ccdesign DoE diamond edges for testing
...
[indx,tf] = listdlg('PromptString','Train hybrid NN or simulate:',...
                     'SelectionMode','single',...
                      'ListString',list);
if indx==2 && tf==1
    [~,~,~,~,~,~,~,~,~,Sext,Sint,isrev]=load_data_hybmod(2); % get the GEM extra- and intracellular 
    % stoichiometric matrix, along with reversibility information
    
    % hybrid FFNN
    structures{1,1}     = {hnetfflayer(27,38,'relu') hnetfflayer(38,38,'relu') hnetfflayer(38,38,'linear') ...
         hnetMetabolicLayer(38,25,Sint,Sext,isrev) }; % FFNN hybrid structure
    structures{1,2}='hybrid_FFNN_1';% name of save file
    structures{1,3}=niter;structures{1,4}=nruns;structures{1,5}=npcs;
    
    % Hybrid LSTM
    structures{end+1,1}     = {hnetfflayer(27,14,'tanh')  hnetLSTMlayer(14,14) ...
         hnetfflayer(14,38,'linear') hnetMetabolicLayer(38,25,Sint,Sext,isrev)}; % LSTM hybrid structure
...
if indx==2 && tf==1
    ...
    fprintf('--------------------------------------!!!!--------------------------------------------\n');
    fprintf('The mat files of each hybrid structure were saved as names given in variable structures{i,2} \n');
