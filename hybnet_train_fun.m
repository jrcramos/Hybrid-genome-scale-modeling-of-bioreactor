...
dopca=2; % 1: do PCA of reacted amount; 2 : identity matrix is generated instead of doing PCA
...
if nargin==1
    ...
    for il=1:hnet.nl
        ...
        elseif strcmp(hnet.layers{1, il}.type ,'Metabolic')
            s=[s '-GEM' '(' num2str(hnet.layers{1, il}.ny) ')'];
        else
            temp=hnet.layers{1, il}.node;temp(1)=upper(temp(1));
            s=[s '-' temp '(' num2str(hnet.layers{1, il}.ny) ')'];
        end
    end
    s=[s '-ODE'  '(' num2str(hnet.layers{1, il}.ny) ')'];
...
[data, isrev, se, si]=readdata(ymode,dopca,npcs);
...
hnet.ymode=ymode;
hnet.dopca=dopca;
hnet.minibatch = 0.9; % minibatch size ]0-1], 1: no minibatch is used
hnet.dropout =  0.05; % regularization stochastic weights dropout [0-1[, 0: no weights are randomly set to zero
hnet.alfa=1e-4;       %ADAM learining rate
hnet.alfa0=hnet.alfa; % inital alfa in linear decay
hnet.alfaf=hnet.alfa; %0.0005; % final alpha in linear decay is used for the learning rate
hnet.nter=niter;
simfile=simfile;      %



resume=0; % if previous fit was stopped, it can restart from the intermediate file runs.mat
rng(1803199136)
hnet=hnetinitw(hnet);
...
ax = gca;ax.TickLength = [0.02, 0.02];ax.LineWidth = 100*0.01;
xlim([0 max(max(runs.itern))*1.02]);ylim([0 max(max(runs.mse))*1.2]);
legend({'MSE train' ' MSE train best' 'MSE crossv' ' MSE crossv best' 'MSE test'},'Location','best')
...
figure('Name','Reacted mass','units','normalized','outerposition',[0.15 0.12 0.4 0.55])
data.ylabel(1) = 'Formed biomass';
% tiledlayout(7,4,'Padding','none','TileSpacing','compact')
...
function [data, isrev, s_extMatch, s_int]=readdata(ymode,dopca,npcs)
% load data
[x,y,accum,formed,vol,age_h,xlabels,ylabels,reacted_masses,s_extMatch,s_int,isrev]=load_data_hybmod(ymode);
...
if dopca==1
    ...
    nscores=npcs;
    [coeff,~,~,~,explained,mu1] = pca(reacted_masses_pca,'NumComponents',nscores,'Algorithm','svd','Centered',false);
    data.coeff=coeff;
...
data.xstd=xstd';
data.ymean=ymean;
data.ystd=repmat(ymax,1,20);%ymaxall;%ystd;
